        ...
        match (
            ...
            (Ready(Some(val1)), Ready(Some(val2))) => {
                ...
                *self.as_mut().project().rhs = val2;
                ...
                Ready(Some(equal_val(val1, val2)))
            }
            (Ready(Some(val1)), Pending) => {
                *self.as_mut().project().lhs = val1;

                Ready(Some(equal_val(val1, *self.as_mut().project().rhs)))
            }
            (Pending, Ready(Some(val2))) => {
                *self.as_mut().project().rhs = val2;

                Ready(Some(equal_val(*self.as_mut().project().lhs, val2)))
            }
...
fn equal_val(lhs: Val, rhs: Val) -> Val {
    use Val::*;
    match (lhs, rhs) {
