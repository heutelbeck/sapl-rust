...
pub(crate) fn eager_and(lhs: &mut Arc<Ast>, rhs: &mut Arc<Ast>) -> Option<Ast> {
    use Ast::*;
...
mod tests {
    ...
    fn test_eager_and_boolean() {
        ...
        assert_eq!(Some(Ast::Boolean(false)), eager_and(&mut lhs, &mut rhs));
        ...
        assert_eq!(Some(Ast::Boolean(true)), eager_and(&mut lhs, &mut rhs));
    }
    ...
    fn test_add_incompatible_types_integer_float() {
        ...
        assert_eq!(None, eager_and(&mut lhs, &mut rhs));
        assert_eq!(None, eager_and(&mut rhs, &mut lhs));
    }
}
