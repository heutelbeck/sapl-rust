...
pin_project! {
    /// Stream returned by the [`eval_lazy_and`] method.
    pub struct EvalLazyAnd<T, U>
    {
        #[pin]
        a: Fuse<T>,
        #[pin]
...
impl<T, U> EvalLazyAnd<T, U> {
    pub(super) fn new(a: T, b: U) -> EvalLazyAnd<T, U>
    where
...
impl<T, U> Stream for EvalLazyAnd<T, U>
where
    T: Stream<Item = Result<Val, String>>,
    U: Stream<Item = T::Item>,
{
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let need_rhs = poll_lhs == Poll::Ready(Some(Ok(Val::Boolean(true))));
        ...
        if *self.as_ref().project_ref().lhs == Ok(Val::Boolean(false)) {
            return Ready(Some(Ok(Val::Boolean(false))));
        }

        Ready(Some(eager_and(
