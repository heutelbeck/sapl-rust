...
mod tests {
    ...
    #[test]
    fn evaluate_target_expr_basic_identifier_expression_action3_expression_step() {
        let pair = SaplParser::parse(Rule::target_expression, "action.array2[(3+1)] == 5")
            .unwrap()
            .next()
            .unwrap();
        let expr = Ast::parse(pair.into_inner()).evaluate(Arc::new(RwLock::new(
    ...
    #[test]
    fn evaluate_target_expr_basic_identifier_expression_action3_wildcard_step1() {
        let pair = SaplParser::parse(Rule::target_expression, "action[*] == [\"value1\", [{\"key\": \"value2\"}, {\"key\": \"value3\"}], [1, 2, 3, 4 , 5]]")
            .unwrap()
            .next()
            .unwrap();
        let expr = Ast::parse(pair.into_inner()).evaluate(Arc::new(RwLock::new(
    ...
    #[test]
    fn evaluate_target_expr_basic_identifier_expression_action3_wildcard_step2() {
        let pair = SaplParser::parse(Rule::target_expression, "action.* == [\"value1\", [{\"key\": \"value2\"}, {\"key\": \"value3\"}], [1, 2, 3, 4 , 5]]")
            .unwrap()
            .next()
            .unwrap();
