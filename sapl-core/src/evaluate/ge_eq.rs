...
pub(crate) fn ge_eq(lhs: &Result<Val, String>, rhs: &Result<Val, String>) -> Result<Val, String> {
    use crate::Val::*;
    match (lhs, rhs) {
        ...
        (_, Err(e)) => Err(e.clone()),
        (lhs, rhs) => Err(format!(
            "Type mismatch. Greater or equals operation expects decimal values, but got: {lhs:#?} and {rhs:#?}"
        )),
    }
}
...
mod tests {
    ...
    fn ge_eq_integer() {
        ...
        assert_eq!(Ok(Val::CompInteger(true, 23)), ge_eq(&lhs, &rhs));

        let (lhs, rhs) = init(42, 42);
        assert_eq!(Ok(Val::CompInteger(true, 42)), ge_eq(&lhs, &rhs));

        let (lhs, rhs) = init(23, 42);
        assert_eq!(Ok(Val::CompInteger(false, 42)), ge_eq(&lhs, &rhs));
    }

    #[test]
    fn ge_eq_comp_integer_with_integer() {
        ...
        assert_eq!(Ok(Val::Boolean(false)), ge_eq(&lhs, &rhs));
    ...
    fn ge_eq_float() {
        ...
        assert_eq!(Ok(Val::CompFloat(true, dec!(2.3))), ge_eq(&lhs, &rhs));

        let (lhs, rhs) = init(dec!(0.1), dec!(0.1));
        assert_eq!(Ok(Val::CompFloat(true, dec!(0.1))), ge_eq(&lhs, &rhs));

        let (lhs, rhs) = init(dec!(2.3), dec!(4.2));
        assert_eq!(Ok(Val::CompFloat(false, dec!(4.2))), ge_eq(&lhs, &rhs));
    }

    #[test]
    fn ge_eq_comp_float_with_float() {
        fn init(
            lhs_b: bool,
            lhs_i: Decimal,
            rhs: Decimal,
        ...
        assert_eq!(Ok(Val::CompFloat(true, dec!(4.2))), ge_eq(&lhs, &rhs));
    ...
    fn ge_eq_lhs_error() {
        ...
        assert_eq!(Err("Fault lhs".to_string()), ge_eq(&lhs, &rhs));
    }
    ...
    fn ge_eq_rhs_error() {
        ...
        assert_eq!(Err("Fault rhs".to_string()), ge_eq(&lhs, &rhs));
    }
    ...
    fn ge_eq_error() {
        ...
        let err = Err(
            "Type mismatch. Greater or equals operation expects decimal values, but got: Ok(\n    String(\n        \"something\",\n    ),\n) and Ok(\n    Integer(\n        42,\n    ),\n)".to_string(),
        );
        assert_eq!(err, ge_eq(&lhs, &rhs));
    }
}
