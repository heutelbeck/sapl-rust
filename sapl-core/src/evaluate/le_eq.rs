...
mod tests {
    ...
    fn le_eq_integer() {
        ...
        assert_eq!(Ok(Val::CompInteger(false, 23)), le_eq(&lhs, &rhs));

        let (lhs, rhs) = init(42, 42);
        assert_eq!(Ok(Val::CompInteger(true, 42)), le_eq(&lhs, &rhs));
    ...
    #[test]
    fn le_eq_comp_integer_with_integer() {
        ...
        assert_eq!(Ok(Val::Boolean(false)), le_eq(&lhs, &rhs));
    ...
    fn le_eq_float() {
        ...
        assert_eq!(Ok(Val::CompFloat(false, dec!(2.3))), le_eq(&lhs, &rhs));

        let (lhs, rhs) = init(dec!(0.1), dec!(0.1));
        assert_eq!(Ok(Val::CompFloat(true, dec!(0.1))), le_eq(&lhs, &rhs));

        let (lhs, rhs) = init(dec!(2.3), dec!(4.2));
        assert_eq!(Ok(Val::CompFloat(true, dec!(4.2))), le_eq(&lhs, &rhs));
    }

    #[test]
    fn le_eq_comp_float_with_float() {
        fn init(
            lhs_b: bool,
            lhs_i: Decimal,
            rhs: Decimal,
    ...
    fn le_eq_lhs_error() {
        ...
        assert_eq!(Err("Fault lhs".to_string()), le_eq(&lhs, &rhs));
    }
    ...
    fn le_eq_rhs_error() {
        ...
        assert_eq!(Err("Fault rhs".to_string()), le_eq(&lhs, &rhs));
    }
    ...
    fn le_eq_error() {
        ...
        let err = Err(
            "Type mismatch. Less or equals operation expects decimal values, but got: Ok(\n    String(\n        \"something\",\n    ),\n) and Ok(\n    Integer(\n        42,\n    ),\n)".to_string(),
        );
        assert_eq!(err, le_eq(&lhs, &rhs));
    }
}
